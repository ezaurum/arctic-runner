...
TITLE → STAGE_INTRO(2.5초) → PLAYING → STAGE_COMPLETE → (다음 스테이지 or GAME_OVER)
                                 ↓               ↕
                     바다표범 충돌 / 시간 초과    PAUSED (포커스 상실 시 자동)
                                 ↓
                            GAME_OVER → TITLE
```
...
| **Sliding** | 빙판(iceHole) 충돌 시 옆으로 밀리며 0.6초간 정지 후 복귀 |
| **Trapped** | 크레바스에 빠짐. 점프 입력으로 탈출 |
| **Dead** | 바다표범 충돌 시 즉사. 게임 오버 |

- 속도는 자동으로 가속되며 최대속도까지 올라감
- 라이프 시스템 없음 — 바다표범 충돌 또는 시간 초과 시 게임 오버

### 장애물

| 종류 | 설명 | 충돌 동작 | 점프 회피 |
|---|---|---|---|
| **얼음 구멍** | 도로 위 구멍 | **slide** — 옆으로 밀리며 0.6초 정지 | O |
| **크레바스** | 넓은 균열 | **trap** — 빠져서 갇힘, 점프로 탈출 | O |
| **바다표범** | 도로 위 물개 | **kill** — 즉사 (게임 오버) | X |

충돌 시 효과음: "딩딩딩" (3연타 상승음)
...
src/
├── main.ts                 # 엔트리 포인트
├── Game.ts                 # 게임 루프 + 상태 머신 (PAUSED 포함)
├── tuning.ts               # 튜닝 도구 UI (tuning.html 전용)
├── config/
...
브라우저에서 `/tuning.html`로 접속하면 GUI 튜닝 도구를 사용할 수 있다:
- 펭귄 속도/점프력/중력 등 슬라이더 조정
- 장애물별 속도 패널티, 스턴 시간, 충돌 동작(slide/trap/kill) 설정
- 카메라, 스테이지, 환경 설정 조정
- JSON 미리보기 + 복사/다운로드
- 우측 iframe에서 게임 실시간 확인
